...
static float feedrate = 300.0, saved_feedrate;
...
int ChangeFilamenTempSet = 200;
bool EnableHeatBedFlag = false;
bool EnableLightFlag = false;
int lightpwm = 0;
#if ENABLED(AUTO_BED_LEVELING_FEATURE)
...
#endif
float auto_leve_grid_point[AUTO_BED_LEVELING_GRID_POINTS * AUTO_BED_LEVELING_GRID_POINTS];
float auto_leve_grid_point_index[AUTO_BED_LEVELING_GRID_POINTS][AUTO_BED_LEVELING_GRID_POINTS];
...
int cfbH = 0, cf_count = 0, cf_flag = true;
float cf_buffer[BLOCK_BUFFER_SIZE] = {0};
...
void manage_change_filament_at_zPos();
void loop() {
...
inline void sync_plan_position_e() {
...
static void run_z_probe() {
  ...
  */
  ...
  float zPosition = -(Z_MAX_LENGTH + 5);
  line_to_z(zPosition);
  st_synchronize();
...
void manage_change_filament_at_zPos() {
  if ( cf_flag == true ) {
    if ( cf_count > 0 ) {
      if ( cf_buffer[cfbH] != 0 && current_position[Z_AXIS] >= cf_buffer[cfbH] ) {

        cf_flag = false;
        cf_count--;
        cf_buffer[cfbH] = 0;
        cfbH = (cfbH + 1) % BLOCK_BUFFER_SIZE;
        enqueue_and_echo_commands_P(PSTR("M600"));

      } else if ( cf_buffer[cfbH] == 0 ) {
        cf_count--;
        cfbH = (cfbH + 1) % BLOCK_BUFFER_SIZE;
        cf_flag = true;
      }
...
inline void gcode_G28() {
  ...
  float Tmatrix[9];
  memcpy(Tmatrix,plan_bed_level_matrix.matrix,9*sizeof(float));
  ...
  refresh_cmd_timeout();
  endstops_hit_on_purpose(); // clear endstop hit flags
  memcpy(plan_bed_level_matrix.matrix,Tmatrix,9*sizeof(float));
  ...
  for (int yCount = 0; yCount < auto_bed_leveling_grid_points; yCount++) {
    ...
    for (int xCount = xStart; xCount != xStop; xCount += xInc) {
      ...
      if (!code_seen('M')) {
        ...
        auto_leve_grid_point[probePointCounter] = measured_z;
      } else {
        measured_z = auto_leve_grid_point[probePointCounter];
      }

      SERIAL_ECHOPAIR("> x = ", xProbe);
      SERIAL_EOL;
      SERIAL_ECHOPAIR("> v = ", yProbe);
      SERIAL_EOL;
      SERIAL_ECHOPAIR("> z = ", measured_z);
      SERIAL_EOL;
      SERIAL_ECHOPAIR("> cx = ", xCount);
      SERIAL_EOL;
      SERIAL_ECHOPAIR("> cy = ", yCount);
      SERIAL_EOL;
      SERIAL_ECHOPAIR("> p = ", probePointCounter);
      SERIAL_EOL;
      ...
      auto_leve_grid_point_index[xCount][yCount] = probePointCounter;
...
inline void gcode_M126() {
  baricuda_valve_pressure = code_seen('S') ? constrain(code_value(), 0, 255) : 255;
}
/**
...
inline void gcode_M128() {
  baricuda_e_to_p_pressure = code_seen('S') ? constrain(code_value(), 0, 255) : 255;
}
/**
...
inline void gcode_M81() {
  ...
  char cmd[20];
  sprintf(cmd, "%s %s.", MACHINE_NAME, MSG_OFF);
  lcd_setstatus(cmd);
  lcd_update();
#endif
}
...
inline void gcode_M121() {
  enable_endstops_globally(false);
}
...
inline void gcode_M300() {
  uint16_t beepS = code_seen('S') ? code_value_short() : 2500;
  uint32_t beepP = code_seen('P') ? code_value_long() : 15;
  if (beepP > 3000) beepP = 3000; // limit to 5 seconds
  buzz(beepP, beepS);
}
...
#define RUNPLAN line_to_destination();
  ...
  //prime by E
  destination[E_AXIS] = lastpos[E_AXIS] - FILAMENTCHANGE_FIRSTRETRACT;
  ...
  else destination[Z_AXIS] = lastpos[Z_AXIS] + FILAMENTCHANGE_ZADD;
#endif
 ...
 // if ( (READ(FILRUNOUT_PIN) ^ FIL_RUNOUT_INVERTING) ) {
  ...
  destination[E_AXIS] += 35;
  line_to_destination(AUTO_FILAMENT_CHANGE_FEEDRATE/2);
  st_synchronize();
...
//  }
  ...
  disable_e3();
  delay(300);
  LCD_ALERTMESSAGEPGM(MSG_USERWAIT);
  ...
  while (!lcd_clicked()) {
    ...
    if (ELAPSED(ms, next_tick)) {
      gcode_M300();
      lcd_quick_feedback();
      next_tick = ms + 2000UL; // feedback every 2s while waiting
    }
    idle(true);
#endif
  } // while(!lcd_clicked)
  ...
  */
  ...
  destination[E_AXIS] = lastpos[E_AXIS] - 50;
  line_to_destination(AUTO_FILAMENT_CHANGE_FEEDRATE);
  st_synchronize();
  delay(300);
  ...
  line_to_destination(1000);
  st_synchronize();
  delay(300);

  destination[E_AXIS] = lastpos[E_AXIS] - 9.5;
  line_to_destination(AUTO_FILAMENT_CHANGE_FEEDRATE);
  st_synchronize();
  delay(300);
  ...
  line_to_destination(AUTO_FILAMENT_CHANGE_FEEDRATE);
  destination[E_AXIS] = lastpos[E_AXIS] - 0.9;
  line_to_destination();
  st_synchronize();
#endif
...
static float puase_lastpos[NUM_AXIS];
inline void gcode_M602() {  // puase move


  st_synchronize();

  puase_lastpos[X_AXIS] = current_position[X_AXIS];
  puase_lastpos[Y_AXIS] = current_position[Y_AXIS];
  puase_lastpos[Z_AXIS] = current_position[Z_AXIS];
  puase_lastpos[E_AXIS] = current_position[E_AXIS];

  destination[E_AXIS] = puase_lastpos[E_AXIS] - 15;
  line_to_destination(5000);
  st_synchronize();
  ...
  destination[Z_AXIS] = puase_lastpos[Z_AXIS] + 10;
  line_to_destination(5000);
  st_synchronize();
...
inline void gcode_M603() {  // resume

  float lastpos[NUM_AXIS];

  st_synchronize();
  destination[X_AXIS] = puase_lastpos[X_AXIS];
  destination[Y_AXIS] = puase_lastpos[Y_AXIS];
  line_to_destination(5000);
  st_synchronize();

  destination[Z_AXIS] = puase_lastpos[Z_AXIS];
  line_to_destination(5000);
  st_synchronize();

  destination[E_AXIS] = puase_lastpos[E_AXIS];
  line_to_destination(5000);
  st_synchronize();
...
inline void gcode_M909() {
  dac_print_values();
}

inline void gcode_M910() {
  dac_commit_eeprom();
}
...
void process_next_command() {
  ...
  manage_change_filament_at_zPos();
