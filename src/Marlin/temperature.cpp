...
*/
    ...
    #if HAS_PID_FOR_BOTH
      if (extruder < 0)
        soft_pwm_bed = bias = d = (MAX_BED_POWER);
      else
        soft_pwm[extruder] = bias = d = (PID_MAX);
    #elif ENABLED(PIDTEMP)
      soft_pwm[extruder] = bias = d = (PID_MAX);
    #else
      soft_pwm_bed = bias = d = (MAX_BED_POWER);
...
float get_pid_output(int e) {
  ...
  float pid_output;
      ...
      if (pid_error[e] > PID_FUNCTIONAL_RANGE ) {
        ...
        pid_output = ( pid_output < 50 ) ? 100: pid_output;
...
ISR(TIMER0_COMPB_vect) {
  ...
  ISR_STATICS(0);
     ...
     */
      ...
      #define MIN_STATE_TIME 32 // MIN_STATE_TIME * 65.5 = time in milliseconds
    #endif
