...
void Config_StoreSettings()  {
  ...
  EEPROM_WRITE_VAR(i, ver); i = 118;
  EEPROM_WRITE_VAR(i, SHORT_BUILD_VERSION); i = 130; 
  ...
  EEPROM_WRITE_VAR(i, plan_bed_level_matrix.matrix);
  ...
  EEPROM_WRITE_VAR(i, ChangeFilamenTempSet);
  EEPROM_WRITE_VAR(i, EnableHeatBedFlag);
  EEPROM_WRITE_VAR(i, EnableLightFlag);
...
void Config_RetrieveSettings() {
  ...
  int i = EEPROM_OFFSET;
  char stored_ver[5];
  char ver[5] = FW_VERSION;
  i = 118;
  EEPROM_READ_VAR(i, stored_ver); 
  
  if (strncmp(ver, stored_ver, 5) != 0) {
    ...
    EEPROM_READ_VAR(i, plan_bed_level_matrix.matrix);
    ...
    EEPROM_READ_VAR(i, ChangeFilamenTempSet);
    EEPROM_READ_VAR(i, EnableHeatBedFlag);
    EEPROM_READ_VAR(i, EnableLightFlag);
...
void Config_ResetDefault() {
  ...
  sprintf(SHORT_BUILD_VERSION,"%s",FW_VERSION);
  ...
  ChangeFilamenTempSet = 200;
  EnableHeatBedFlag = false;
  EnableLightFlag = false;
  ...
  char tt[24];
  int i = 0;
  i = 106; EEPROM_READ_VAR(i,tt);
  for (int i = 100 ; i < 756 ; i++) {
    EEPROM.write(i, 0);
  }
  i = 106; EEPROM_WRITE_VAR(i,tt);
