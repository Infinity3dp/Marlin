...
static void lcd_set_custom_characters(
  ...
  byte clock[8] = {
    ...
    if (progress_bar_set != char_mode) {
      ...
      lcd.createChar(LCD_STR_SD[0], sd);
...
unsigned lcd_print(char c) { return charset_mapper(c); }
  ...
  void lcd_scroll(int col, int line, const char* text, int len, int time) {
   ...
   void logo_lines(const char *extra) {
    lcd.setCursor(2, 1); lcd.print('\x00'); lcd_printPGM(PSTR( "-------------" )); lcd.print('\x01');
    lcd.setCursor(2, 2); lcd_printPGM(PSTR("| ")); lcd.print('\x04');lcd.print('\x05');  lcd_printPGM(PSTR("3DPrinter |"));
    lcd.setCursor(2, 3); lcd.print('\x02'); lcd_printPGM(PSTR( "-------------" )); lcd.print('\x03');
  }

static int status_flag = 0;
static int flag = 0;

  void __logo_lines(const char *extra) {
    ...
    lcd.setCursor(4, 1); lcd_print(MACHINE_NAME);
...
if( flag == 1 && EnableHeatBedFlag == true ){
      flag = 0;    
      #if EXTRUDERS > 1
            lcd.setCursor(4, 3); LCD_TEMP(degHotend(1), degTargetHotend(1), LCD_STR_THERMOMETER[0]);

      #else
            lcd.setCursor(4, 3); LCD_TEMP(degBed(), degTargetBed(), LCD_STR_BEDTEMP[0]);

      #endif
}else{
     flag = 1;
    lcd.setCursor(4, 3); LCD_TEMP(degHotend(0), degTargetHotend(0), LCD_STR_THERMOMETER[0]);
}
  ...
  }else if (!(sd_status  && lcd_detected() )){
        lcd_printPGM(PSTR("NO SD"));
      }else{
        lcd_printPGM(PSTR("Ready"));
      }
      
}

int bootscreenflag = 0;
  void bootscreen() {
    bootscreenflag = 1;
    ...
    lcd.createChar(4, s_f);
    lcd.createChar(5, s_s);
    ...
    #define CENTER_OR_SCROLL(STRING,DELAY) \
      ...
      if (LCD_EXTRA_SPACE >= strlen(STRING_SPLASH_LINE1) + 1) {
        ...
        //
        logo_lines(STRING_SPLASH_LINE1);
...
void lcd_implementation_drawedit_for_z_offset(const char* pstr, const char* value=NULL) {
...
#define lcd_implementation_drawmenu_cancel(sel, row, pstr) lcd_implementation_drawmenu_generic(sel, row, pstr, LCD_STR_UPLEVEL[0], LCD_STR_UPLEVEL[0])
